 ...
 * presenter controls are the one thing that ruins them: a "Reset demo data"
 * button makes a working system read as staged, which is the opposite of the
 * point. `?clean=1` hides them.
 *
 ...
 *
 * The choice is sticky for the browser tab, so it survives a reload and any
 * navigation that drops the query string. Closing the tab clears it.
 *
 * The companion supervisor demo carries the same flag, so one capture run can
 * point both apps at the same URL convention.
 */
